...
def save_data():
    ...
    dat.to_csv(dirs+'code.csv',encoding='utf8')
    inuse = []
    ...
    for code in dat['code'].values:
        ...
        try:
            ...
            if _data_ is not None:
                _data_.to_csv(dirs+'%s.csv'%code,encoding='utf8')
                if _data_.size>0 and _data_.index[0] in ct._start_range and _data_.index[-1] in ct._end_range:                          #筛选一次代码，使用头尾都包含的代码
                    inuse.append(code)
        except IOError:
            continue  #不行的话还是continue
    #print len(inuse)
    _df_inuse = DataFrame(inuse,columns={'code'})
    _df_inuse.to_csv(dirs+'code_inuse.csv',encoding='utf8')

#从网络中更新数据,code 必须为str，dat中的为int
def refresh_data(_start_ ='2015-08-01',_end_ = ct._TODAY_):
    dat = pd.read_csv(dirs+'code.csv',index_col=0,dtype={'code': str},encoding='utf8')
    inuse = pd.read_csv(dirs+'code_inuse.csv',index_col=0,dtype={'code': str},encoding='utf8')
    new_inuse = []
        ...
        try:
            ...
            if _data_ is not None and _data_.size != 0:
                if os.path.exists(filename):
                    _data_.to_csv(filename, mode='a', header=None,encoding='utf8')
                else:
    ...
    _df_inuse.to_csv(dirs+'code_new_inuse.csv',encoding='utf8')
...
def read_data():      
    ...
    for code in dat['code'].values:
        ...
        print((i,code))
        try:
            df = pd.read_csv(dirs+'%s.csv'%code,index_col=0,parse_dates=[0],dtype={'code': str},encoding='utf8')  #parse_dates直接转换数据类型，不用再重新狗再累
            if df is not None:
                dic[code] = df
        except IOError:
            continue    #不行的话还是continue
...
def get_universe():
    try:
        dat = pd.read_csv(dirs+'code_inuse.csv',dtype={'code': str},index_col=0,encoding='utf8')
    except Exception: 
        dat = ts.get_industry_classified()
...
def get_data(code):
    try:
        dat = pd.read_csv(dirs+'%s.csv'%code,index_col=0,dtype={'code': str},parse_dates=[0],encoding='utf8')  #parse_dates直接转换数据类型，不用再重新狗再累
    except Exception:
        dat = None
    return dat
def get_macd(df):
...
# save_data()
# refresh_data()
# df = pd.read_csv('d:/data/600848.csv',index_col=0,parse_dates=[0],encoding='utf8')
# da = get_macd(df)
#plt_macd(df,da)
#_data_ = pd.read_csv('d:/data/600848.csv',index_col=0,encoding='utf8')
#dic = read_data()
...
def temp2():
    dat = pd.read_csv(dirs+'code.csv',dtype={'code': str},index_col=0,encoding='utf8')
    inuse = []   
    i = 0
    for code in dat['code'].values:
        i+= 1
        print((i,code))
        try:
            _data_ = pd.read_csv(dirs+'%s.csv'%code,index_col=0,dtype={'code': str},parse_dates=[0],encoding='utf8')   #默认取3年，code为str，start无效的,start 和end若当天有数据则全都取
            if _data_ is not None:
...
def temp():
    dat = pd.read_csv(dirs+'code.csv',dtype={'code': str},index_col=0,encoding='utf8')
    inuse = pd.read_csv(dirs+'code_inuse.csv',dtype={'code': str},index_col=0,parse_dates=[0],encoding='utf8')
    new_inuse = []
    ...
    for code in dat['code'].values:
            ...
            #print i,code
            try:
                _data_ = pd.read_csv(dirs+'%s.csv'%code,index_col=0,dtype={'code': str},parse_dates=[0],encoding='utf8')  #默认取3年，start 8-1包括
...
def change_type_to_yahoo():
    # dirs='d:/quant/data2'
    inuse = pd.read_csv(dirs+'code_inuse.csv',index_col=0,parse_dates=[0],encoding='utf8')
    inuse.to_csv(dirs+'code_inuse.csv',encoding='utf8')
    re_columns ={'high':'High','low':'Low','open':'Open','close':'Close','volume':'Volume','price_change':'Adj Close'}  
    i=0
    for code in inuse['code'].values:
        i+= 1
        print((i,code))
        _data_ = pd.read_csv(dirs+'%s.csv'%code,index_col=0,parse_dates=[0],encoding='utf8')  #默认取3年，start 8-1包括
        _data_=_data_.rename(columns=re_columns)
        _data_.index.name = 'Date'
        _data_.to_csv(dirs+'%s.csv'%code,columns=['Open','High','Low','Close','Volume','Adj Close'],date_format="%Y-%m-%d",encoding='utf8')
...
def bigVolume(scope=15,v_times=5,t_percent=20):
    ...
    for code in inuse['code'].values:
        try:
             _data_ = pd.read_csv(dirs+'%s.csv'%code,index_col=0,parse_dates=[0],encoding='utf8')   #默认取3年，code为str，start无效的,start 和end若当天有数据则全都取
...
# bigVolume()
# _data_ = pd.read_csv('d:/quant/data/000088.csv',index_col=0,parse_dates=[0],encoding='utf8')   #默认取3年，code为str，start无效的,start 和end若当天有数据则全都取
# _data_.plot()
# plt.show()
